<template>
  <div class="bar" :class="[
  {
    ...
    <svg class="shading" viewBox="0 0 100 100" preserveAspectRatio="none">
      <g >
        ...
        <g class="borders" fill="transparent" stroke="var(--gridline-color)" stroke-width="2">
          ...
          <!-- right -->
          <path v-if="barData.rightBarline.includes('||') || barData.rightBarline === '|:' || barData.rightBarline === ':|'" d="M 98 0 98 100" stroke-width="2"></path>
    ...
    <div class="to-segno-diagram" v-if="barData.classes.includes('to-segno')">
      <div>To</div>
...
<style>
  ...
  .to-segno-diagram .segno-symbol {
    height: 24px;
    margin: 0 0 0 -4px;
  }
  .bar.is-rightmost .to-segno-diagram {
    left: 100%;
    bottom: 0;
    padding-left: 4px;
  }
